...
IRCClient = function() {
    ...
    self.nick = function(nickname) {
        lastnicktry = nickname;
        
        if(lastnicktry.indexOf(':') !== -1) {
            lastnicktry = lastnicktry.split(':');
            lastnicktry = lastnicktry[0];
        }
        
        send("NICK", nickname)
    }
    self.lastnicktry = function() {
        return lastnicktry;
    }
    ...
    self.muteaccept = function(nickname) {
	send('ACCEPT', nickname)
    }	
    self.pass = function(password) {
	    send('PASS', password)
    }
/*    self.ctcp = function(destination, message) {
        send('PRIVMSG', destination + ' :\01' + message + '\01')
    }
    */
    ...
    self.mode = function(nickname, mode) {
        send('MODE ' + nickname + ' ' + mode)
    }
